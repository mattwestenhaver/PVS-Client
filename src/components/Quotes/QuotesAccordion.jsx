...
class QuotesAccordion extends React.Component {
    ...
    archiveQuote(quote) {
        auth.archiveQuote(quote).then(response => {
    ...
    activateQuote(quote) {
        auth.activateQuote(quote).then(response => {
            if (response.data.success) {
                this.getQuotes();
    ...
    render() {
        ...
        return (
            <div>
                <Button className="notification-test" size="huge" color="red" onClick={this.testEmails.bind(this)}>
                ...
                <div>
                    {this.state.viewArchived === false ? (
                        <div>
                            <h2>Active Quotes</h2>
                            <Button size="huge" onClick={this.switchView.bind(this, true)}>
                        ...
                        <div>
                            ...
                            <Button size="huge" onClick={this.switchView.bind(this, false)}>
                                View Active Quotes
                            </Button>
                        </div>
                    )}
                                ...
                                <Accordion styled key={q._id} className="quote-container">
                                    ...
                                    <Accordion.Content active={activeIndex === q._id}>
                                        <div>
                                            ...
                                            <p>{q.comments}</p>
                                            {q.archived ? (
                                                <Button size="huge" onClick={this.activateQuote.bind(this, q)}>
                                                    Activate
                                                </Button>
                                            ) : (
                                                <Button size="huge" onClick={this.archiveQuote.bind(this, q)}>
                                                    Archive
                                                </Button>
                                            )}
                                        </div>
